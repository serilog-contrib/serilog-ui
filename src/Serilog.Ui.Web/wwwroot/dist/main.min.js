"use strict";(function(n){const t=function(){n(".js-fullheight").css("height",n(window).height());n(window).resize(function(){n(".js-fullheight").css("height",n(window).height())})};t();n(".sidebar-collapse").on("click",function(){n("#sidebar").toggleClass("active")});n("body").on("click",".page-link",function(t){t.preventDefault();n("#page").val(n(this).attr("data-val"));fetchData()});n("#logCount").on("change",function(){n("#page").val("1");fetchData()});n("#logFilter").on("change",function(){n("#page").val("1");fetchData()});n("#search").on("keypress",function(t){t.which===13&&(n("#page").val("1"),fetchData())});n("#search").on("input",function(){n(this).val()===""&&(n("#page").val("1"),fetchData())});n("body").on("click",".modal-trigger",function(t){t.preventDefault();const u=n("#messageModal"),r=u.find(".modal-body"),f=n(this).attr("data-type");let i=n(this).find("span").text();if(f==="xml")i=n(this).find("span").html(),i=formatXml(i,"  "),n(r).removeClass("wrapped");else if(f==="json"){const t=JSON.parse(i);i=JSON.stringify(t,null,2);n(r).removeClass("wrapped")}else n(r).addClass("wrapped");r.find("pre").text(i);u.modal("show")});n("#saveJwt").on("click",function(){const t=n(this).data("saved");if(t.toString()==="false"){const t=n("#jwtToken").val();if(!t)return;sessionStorage.setItem("serilogui_token",t);n("#jwtToken").remove();n("#tokenContainer").text("*********");n(this).text("Clear");n(this).data("saved","true");n("#jwtModalBtn").find("i").removeClass("fa-unlock").addClass("fa-lock");fetchData();return}sessionStorage.removeItem("serilogui_token");n(this).text("Save");n(this).data("saved","false");n("#tokenContainer").html('<input type="text" class="form-control" id="jwtToken" autocomplete="off" placeholder="Bearer eyJhbGciOiJSUz...">');n("#jwtModalBtn").find("i").removeClass("fa-lock").addClass("fa-unlock")})})(jQuery);const routePrefix={url:"",set setUrl(n){this.url=n}},init=n=>{n.authType==="Cookie"?($("#jwtModalBtn").remove(),sessionStorage.removeItem("serilogui_token")):initTokenUi(),routePrefix.setUrl=n.routePrefix,fetchData()},fetchData=()=>{const n=$("#logTable tbody"),t=$("#page").val(),i=$("#count").children("option:selected").val(),r=$("#level").children("option:selected").val(),u=escape($("#search").val()),f=`/${routePrefix.url}/api/logs?page=${t}&count=${i}&level=${r}&search=${u}`,e=sessionStorage.getItem("serilogui_token");$.ajaxSetup({headers:{Authorization:e}});$.get(f,function(t){$("#totalLogs").html(t.total);$("#showingItemsStart").html(t.page);$("#showingItemsEnd").html(t.count);$(n).empty();t.logs.forEach(function(t){let i="";t.exception!=undefined&&(i=`<a href="#" title="Click to view" class="modal-trigger" data-type="text">
                        View <span style="display: none">${t.exception}</span>
                    </a>`);const r=`<tr class="${t.level}">
                <td class="text-center">${t.rowNo}</td>
                <td class="text-center"><span class="log-level text-white ${levelClass(t.level)}">${t.level}</span></td>
                <td class="text-center">${formatDate(t.timestamp)}</td>
                <td class="log-message">
                    <span class="overflow-auto"><truncate length="100">${truncateString(t.message,100)}</truncate></span>
                </td>
                <td class="text-center">
                    ${i}
                 </td>
                <td class="text-center">
                    <a href="#" class="modal-trigger" title="Click to view" data-type="${t.propertyType}">
                    View
                        <span style="display: none">${t.properties}</span>
                    </a>
                </td>
            </tr>`;$(n).append(r)});paging(t.total,t.count,t.currentPage)}).fail(function(n){if(n.status===403)console.log(n),alert("You are not authorized you to access logs.\r\nYou are not logged in or you don't have enough permissions to perform the requested operation.");else if(n.status===500){const t=JSON.parse(n.responseJSON.errorMessage);alert(t.errorMessage)}else alert(n.responseText)})},levelClass=n=>{switch(n){case"Verbose":case"Debug":return"bg-success";case"Information":return"bg-primary";case"Warning":return"bg-warning";case"Error":return"bg-danger";default:return""}},formatDate=n=>{var t=new Date(n);return`${(t.getMonth()+1).toString().padStart(2,"0")}/${t.getDate().toString().padStart(2,"0")}/${t.getFullYear().toString().padStart(4,"0")}
