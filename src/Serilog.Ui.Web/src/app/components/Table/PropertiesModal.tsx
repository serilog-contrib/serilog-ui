import type { EncodedSeriLogObject } from 'types/types';
import {
  Accordion,
  Box,
  Button,
...
const RenderProps = memo(
  ({
    ...
    const { additionalColumn, removeProperties } = useColumnsInfo(
      name,
      logPropertyType,
    );
    ...
    const defaultAccordionValue = expandDropdownsByDefault
      ? propertyName
      : null;
      ...
      return (
        <Accordion defaultValue={defaultAccordionValue} style={{ order: 1 }}>
          <Accordion.Item value={propertyName}>
            <Accordion.Control icon={<IconCodeDots />}>
              {propertyName}
            </Accordion.Control>
            <Accordion.Panel>
              <Suspense>
  ...
  return (
    <>
      ...
      <Box display='grid' style={boxButton}>
        <Button
          size='compact-sm'
          disabled={!Object.keys(rest).length}
          onClick={open}>
          {title}
        </Button>
      </Box>
