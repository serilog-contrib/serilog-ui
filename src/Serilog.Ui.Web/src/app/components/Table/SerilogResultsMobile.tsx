...
const MobileSkeleton = () => {
  ...
  return [...Array.from({ length: 4 }).keys()].map((value) => (
    <Skeleton
      h='14em'
      key={value}
      radius='none'
...
const LogCard = memo(
  ({ log, isUtc }: { log: EncodedSeriLogObject; isUtc?: boolean }) => {
    const theme = useMantineTheme();
    const { colorScheme } = useMantineColorScheme();

    const colorByLogLevel = getBgLogLevel(
      theme,
      colorScheme,
      LogLevel[log.level],
    );
...
const SerilogResultsMobile = () => {
  ...
  return (
    <SimpleGrid
      ...
      {data?.logs.length
        ? data.logs.map((log) => (
            <LogCard key={log.rowNo} isUtc={isUtc} log={log} />
          ))
        : null}
    </SimpleGrid>
  );
