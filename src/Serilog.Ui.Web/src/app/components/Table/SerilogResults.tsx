...
const DesktopSkeleton = () =>
  [...Array.from({ length: 10 }).keys()].map((k) => (
    <Table.Tr key={k} h='4em'>
      {[...Array.from({ length: 8 }).keys()].map((k) => (
        <Table.Td key={k} p='0'>
...
const SerilogResults = () => {
  ...
  const getCellColor = useMemo(
    () => (logLevel: string) =>
      getBgLogLevel(theme, colorScheme, LogLevel[logLevel]),
    [colorScheme, theme],
  );
  ...
  return (
    <Table.ScrollContainer minWidth={1250}>
      <Table
        ...
        <TableHead />
        <Table.Tbody
          className={isFetching ? classes.skeletonDesktopTableCell : ''}>
          {!isFetching &&
            isObjectGuard(data) &&
